...
public class Main implements Java8Listener {
	...
	public void enterWildcard(WildcardContext ctx) {
		// TODO Auto-generated method stub
		try {
			if ((ctx.getChild(0).getText() + ctx.wildcardBounds().getChild(0))
					.equals("?extends"))
				bw.write("out ");
			else if ((ctx.getChild(0).getText() + ctx.wildcardBounds()
					.getChild(0)).equals("?super"))
				bw.write("in ");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void enterSingleTypeImportDeclaration(
		...
		try {
			...
			if (firstImport) {
				...
				if (packageName.equals(ctx.typeName().packageOrTypeName()
						.getText())) {
					bw.write(",\n" + ctx.typeName().getChild(2).getText());
				} else {
					bw.write("\n}\n");
